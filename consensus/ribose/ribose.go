...
func (r *Ribose) trySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	msg := types.NewMessage(header.Coinbase, &systemcontracts.RiboseContractAddr, nonce, fee, math.MaxUint64, new(big.Int), new(big.Int), new(big.Int), data, nil, true)
...
func (r *Ribose) getSortedValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
	...
	msg := types.NewMessage(header.Coinbase, &systemcontracts.RiboseContractAddr, 0, new(big.Int), math.MaxUint64, new(big.Int), new(big.Int), new(big.Int), data, nil, false)
...
func (r *Ribose) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	// call contract
	nonce := state.GetNonce(header.Coinbase)
	msg := types.NewMessage(header.Coinbase, &systemcontracts.RiboseContractAddr, nonce, new(big.Int), math.MaxUint64, new(big.Int), new(big.Int), new(big.Int), data, nil, true)
	if _, err := executeMsg(msg, state, header, newChainContext(chain, r), r.chainConfig); err != nil {
