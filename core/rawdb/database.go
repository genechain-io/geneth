...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("ribose-")) && len(key) == 7+common.HashLength:
			riboseSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Ribose snapshots", riboseSnaps.Size(), riboseSnaps.Count()},
