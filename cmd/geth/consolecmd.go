...
// localConsole starts a new geneth node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc geneth: %v", err)
	}
...
// remoteConsole will connect to a remote geneth instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running geneth instance and start the JavaScript console
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote geneth: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		...
		// Backwards compatibility with geneth < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new geneth node, attaches an ephemeral JavaScript
